class ServicesController < ApplicationController
  ...
  def index
    if params[:search]
      @services = Service.search(params[:search]).page(params[:page]).per(5)
    else
      @services = Service.all.page(params[:page]).per(5)
    end
  end
  ...
  private
    ...
    def service_params
      params.require(:service).permit(:user_id, :description, :status_id)
    end
end
